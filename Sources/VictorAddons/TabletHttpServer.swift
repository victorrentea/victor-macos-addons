...
case testTerminalFont
    ...
    case testLiveCaptionsSay(String?, String?)
        ...
        /// Show an FX card now with an optional "?name=" presser (defaulting to a
        /// sample), without needing the daemon or the soundboard (test hook).
        case testFx(String?)
    ...
    /// Show an FX card now; the argument is the optional "?name=" presser.
    ...
    static func route(forPath path: String) -> Route {
        ...
        case "/test/fx":
            return .testFx(queryItems.first(where: { $0.name == "name" })?.value)
