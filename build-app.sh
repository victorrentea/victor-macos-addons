...
for SIZE in 16 32 64 128 256 512; do
    sips -z $SIZE $SIZE "$DIR/wispr-flow/icon_chat.png" --out "$ICONSET/icon_${SIZE}x${SIZE}.png" >/dev/null 2>&1
    DOUBLE=$((SIZE * 2))
    if [ $DOUBLE -le 1024 ]; then
        sips -z $DOUBLE $DOUBLE "$DIR/wispr-flow/icon_chat.png" --out "$ICONSET/icon_${SIZE}x${SIZE}@2x.png" >/dev/null 2>&1
    fi
done
ICNS_FILE="$DIR/wispr-flow/AppIcon.icns"
