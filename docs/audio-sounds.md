...
**🎙️ "Listening to: …" (`MicSourceAnnouncer`, 2026-09-23).** When the *system default input* changes — the DJI receiver plugged into USB-C or pulled out, or the input switched in Sound settings — a green `BottomTabBanner` tab (the `BellCard`/`FxCard` surface) rises on the screen under the mouse reading `🎙️ Listening to: <CoreAudio name>`, holds 2.5 s and falls away; click-through, non-activating. Two listeners (`DefaultInputDevice` + `Devices`) feed a 0.6 s settle timer so a plug-in burst yields one tab with the final device; the device present at launch is the baseline and is never announced; A → B → A inside the settle window announces nothing. Log line: `🎙️ Mic source → <name>` in `/tmp/victor-macos-addons.log`. **Caveat:** this is the macOS default input, which Wispr Flow follows on *Auto-detect* — but neither Whisper here nor Walkie Talkie's own recorder does: both resolve their mic from `~/.walkie-talkie/mic/choice` + the `MicRoster` ladder (XLR ▸ DJI Rx ▸ DJI TX ▸ …) and use the system default only when none of those is present.

**🎧 The WH-1000XM3 is never the default input (`HeadphoneMicGuard`, 2026-09-23).** When the headset's mic is open the Bluetooth link drops from A2DP to HFP (16 kHz mono, "phone call" sound), and macOS makes it the default input on connect. `MicSourceAnnouncer`'s settle (and its launch baseline) first calls `HeadphoneMicGuard.enforce()`: if the default input's name contains `WH-1000X`, it is moved to the first present `MicRoster` mic, else any other input. Log line: `🎧 … → moved to …`. It cannot stop an app that selects the headset mic by name (Teams/Zoom mic set to the headset instead of the system default).
