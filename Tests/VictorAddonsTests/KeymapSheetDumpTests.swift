...
/// Not an assertion — a darkroom. Renders the real ⌥ / ⌥⇧ / ⌃⌥ sheets to PNG so
/// the drawing can be looked at instead of reasoned about.
final class KeymapSheetDumpTests: XCTestCase {
    func testDumpSheets() throws {
        ...
        for layer in EmojiKeyLayer.Layer.allCases {
            ...
            let image = renderer.render(outputs: EmojiKeyLayer.snapshot(layer).bindings)
