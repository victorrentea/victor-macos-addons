...
- **Mute 🎶 (auto)** — drops `🔊OS Output` device volume to 1% during Wispr Flow dictation, restores on stop. `CoreAudioManager` polls `kAudioProcessPropertyIsRunningInput` on `com.electron.wispr-flow.*` every 300ms. While Wispr is recording AND we haven't already muted, each tick taps the loopback for ~150ms to measure RMS/peak energy (thresholds RMS 0.0002 / peak 0.0005); on the first poll where audio is detected, the current device volume is captured into `originalVolume` and the device's `kAudioDevicePropertyVolumeScalar` is set to 0.01. On Wispr `1→0` we restore `originalVolume` — the loopback check is **not** consulted on restore (a muted device reads silent and would falsely cancel restore). State (`volumePushedDown`, `originalVolume`) lives entirely on the serial poll queue, so reads/writes are sequential. No Mouse 5 / ESC hooks — we follow Wispr's actual recording state, so behavior is independent of how Wispr was triggered (Mouse 5, right Opt-Cmd, hotkey, UI button, VAD timeout, ESC cancel). Caveats: an app crash mid-mute leaves the volume at 1% until next Wispr cycle; a manual volume-slider tweak during dictation is overwritten on restore.
