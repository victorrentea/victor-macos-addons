...
- **🖥️ Auto display arrangement (projector workflow, `DisplayArrangementManager`)** — reshapes the displays automatically when a projector is plugged in or out, so Victor never re-does the venue setup by hand. Detection is `CGDisplayRegisterReconfigurationCallback` (fires on every hot-plug / mode / mirror change), **debounced ~1.2 s** so the hardware settles before we read + apply; we act only when the *scene* — `(hasProjector, hasASUS)` — actually changes, so re-applying our own layout never loops (our own `CGCompleteDisplayConfiguration` callbacks compute the same scene and no-op, plus an `isApplying` guard + 1 s cooldown). Roles are resolved **live, not from a frozen profile**, so it works with any venue's projector (different EDID each time): built-in = **Retina** (`CGDisplayIsBuiltin`); `NSScreen.localizedName` contains "ASUS" = the **ASUS MB166C** portable (its vendor/model is cached as a fallback for mirror-set relaunches where the name isn't exposed); anything else external = the **projector**. **Projector connected** → the projector *mirrors* the Retina and the Retina drops to **1920×1080** (a true 1080p signal — "what's projected to the room"); if the ASUS is also present it becomes the **primary/main** display (origin `(0,0)`) on the right, Retina extended to **its left** (origin `-1920,0`). **No projector** → the standard rig: Retina **main** at its native mode, ASUS (if present) extended to the Retina's **right**. Applying is one atomic Quartz transaction (`CGBegin…/CGConfigureDisplayWithDisplayMode` + `CGConfigureDisplayMirrorOfDisplay` + `CGConfigureDisplayOrigin` + `CGCompleteDisplayConfiguration(.permanently)`) — no external tools, **no Screen-Recording permission** needed. The Retina's user-normal (native HiDPI) mode is captured the first time we observe a projector-free state and restored verbatim on revert (never guesses the scaled res). 1080p is found via `CGDisplayCopyAllDisplayModes` with `kCGDisplayShowDuplicateLowResolutionModes`, preferring a true non-HiDPI 1920×1080 @ 60 Hz. It **does not auto-apply on launch** (only on subsequent changes) so starting the app never reshuffles a happy layout; the menu item **🖥️ Fix display layout** (under the ⭐️/extras submenu) and the `GET /test/projector` hook force it on demand. On each apply it flashes the standard bottom-left `StatusBanner` (e.g. "🖥️ Projector: mirror + ASUS primary"). Test hook: `GET /test/projector` force-applies now and returns a JSON snapshot of the detected displays + resolved scene + whether a 1080p Retina mode is available.
