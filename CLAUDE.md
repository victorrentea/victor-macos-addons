...
| [docs/lid-awake.md](docs/lid-awake.md) | 🔋 Claude prevents sleep (`LidAwake` + `LidAwakePolicy` + `ClaudeActivity`): holds the Mac awake on battery with the lid shut **only while a Claude session is working** (a live `caffeinate` with a `claude` parent), via the kernel `SleepDisabled` flag (**why `caffeinate` cannot do this**), the narrow sudoers rule it needs, the 10 s lub-dub from the 💓 SFX that doubles as the proof (system output parked at 100% while it runs), the 🫀 flatline that announces the release, soft release vs. the hard 20% floor |
