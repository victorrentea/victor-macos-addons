...
| doc | covers |
|---|---|
| [docs/transcription.md](docs/transcription.md) | 💬 Whisper live transcription (AC-power-driven), heartbeat + output watchdog, 🎙️ "Ce tocmai am spus" picker (⌘⌃V, `TranscriptDistiller`), **💬📺 live subtitles (⌘⌃U, `LiveCaptions` + `CaptionStream`) that rewrite their own tail as later chunks hear it better**, the `whisper-transcribe` Python engine (PortAudio discipline, supervised threads, quiet crashes) |
