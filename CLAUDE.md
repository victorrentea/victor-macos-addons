...
- `GET /test/email` — force one 📬 Flux-inbox poll now, **bypassing the battery gate**. NB this is not read-only: a poll that finds new mail **claims it and launches the full agent** (Terminal + `claude` + reply email). It returns a JSON snapshot: `on_battery_now`, `last_outcome`, `last_match_count`, `watermark`, `last_poll_at`, plus the configured `trusted_sender` / `match` rule. Blocks up to 2.5 s for the round trip (AgentMail answers in ~0.5 s) so the response reflects *this* poll; on timeout it returns `"status":"pending"` and the result still lands in the log and banner
