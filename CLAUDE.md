...
| [docs/mouse-reconnect.md](docs/mouse-reconnect.md) | 🖱️ reconnects Victor's own Bluetooth mouse when paired-but-not-connected while nearby (`MouseAutoReconnect`): the address allow-list (six identically-named "Logi M650 L" pairings on this Mac, only one his), why the address survives a factory reset, the polled `openConnection` retry |
