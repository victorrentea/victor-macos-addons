...
| [docs/lid-awake.md](docs/lid-awake.md) | 🔋 Awake Lid Closed (`LidAwake` + `LidAwakePolicy`): keeps the Mac running on battery with the lid shut and nothing attached, via the kernel `SleepDisabled` flag (**why `caffeinate` cannot do this**), the narrow sudoers rule it needs, the 5 s beep that doubles as the proof, the 20% floor |
