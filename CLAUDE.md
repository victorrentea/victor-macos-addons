...
- **`AddonSounds`** keeps the four sounds that must ring even with the effects
  app stopped: the break gong, `LidAwake`'s heartbeat/flatline, 🏁
  "over and out", and the 🚪 sleep chime. Everything else is played over there.
...
**Tech**: Swift, AppKit, AVFoundation, Swift Package Manager; Python 3.12 sidecars; `claude -p` on the subscription (sonnet, for the 🎙️ distillation)
...
| doc | covers |
|---|---|
| [docs/transcription.md](docs/transcription.md) | 🎬 **Live subtitles** (ElevenLabs Scribe v2 Realtime, menu switch, off by default — the *only* thing here that leaves this Mac), 💬 Whisper live transcription (AC-power-driven, local, unchanged), heartbeat + output watchdog, 🎙️ "Ce tocmai am spus" picker (**a menu row since 2026-09-17, no shortcut** — it gave ⌘⌃V to the clipboard→notes append; `TranscriptDistiller`), the `whisper-transcribe` Python engine (PortAudio discipline, supervised threads, quiet crashes) |
...
| [docs/hotkeys-launchers.md](docs/hotkeys-launchers.md) | 🤖 terminals ⌘⌃C/Q/T + quarter placement + tiling, 📝 ⌘⌃N notes doc, 📕⌘⌃K / 📧⌘⌃G / 📅⌘⌃L openers, 📤 ⌘⌃M Reminder mail (`ReminderMail`; the ✉️ TO DO draft/`GmailCompose` was deleted 2026-09-14), 🤖 ⌘⌃P selection → agent prompt + its clipboard-only menu row, 📋 ⌘⌃Z/E/R snippet paste, 🎧 ⌘⌃F focus playlist toggle (`FocusPlaylist`), 🔍 ⌘⌃U magnifier-style flip (`ZoomLensWatch` — decides whether a Zoom share carries the magnification at all), 🎯 ⌘⌃D Walkie Talkie bind, ⌘⌃⌥D dark mode, Wheel×2 given up to Walkie Talkie, 🔎 Cmd+scroll terminal font zoom (`TerminalZoomSizeLock`), 🔄 mouse-wheel scroll reversal (`ScrollReversal`, replaces Scroll Reverser, checkbox under Extra, default ON), ⬇️ back thumb button → Return (`BackButtonEnter`, replaces LinearMouse) |
| [docs/clipboard-history.md](docs/clipboard-history.md) | 📋 ⌘⇧V clipboard history — Flycut's gesture **with images** (`ClipboardHistoryOverlay` + `ClipboardHistoryStore` + `ClipboardHistoryPolicy`): the bezel, a quarter-of-the-screen image, "12 minutes ago" instead of the source app, pixels on disk in Caches and never in memory, the three self-cleaning ceilings, why Flycut has to be quit |
| [docs/session-notes.md](docs/session-notes.md) | 📝 ⌘⌃S selection-or-clipboard → notes (`SessionNotesAppender`), 📝 ⌘⌃V clipboard→notes (took the key from the 🎙️ picker on 2026-09-17, was ⌃⌥V), bottom-left notes pills: markers, rising/sinking exits, `HoverMotionGate` |
| [docs/audio-sounds.md](docs/audio-sounds.md) | Mute 🎶 during Wispr dictation (`CoreAudioManager`), pauza muzicii din Chrome pe durata dictării (`ChromeBridge` :8766 + `chrome-extension/`, fronturi din listener-ul pe lista de procese audio), tablet→Mac sound routing + `SoundsManifest` anti-drift, **Bluetooth speakers**: BT wake-up compensation (**moved to `victor-effects`**, together with `SoundsManifest` and `BluetoothKeepAlive`), `AddonSounds` (the four sounds this app still plays itself), `OutputRouter` (one owner of the default output: the `Bose > JBL` ladder that takes over on connect and hands back when the headset comes off, plus the blocklist that keeps the DJI lavalier — a microphone macOS publishes as an output — off **both** default outputs; no-polling CoreAudio listeners) |
...
| [docs/research-proof.md](docs/research-proof.md) | 🔬 Fact-check the last 10 min (`ResearchProofLauncher` + `research-proof.sh`): settle → 10-min window → claims → researcher subagents → **quote proven by substring search** (`verify-quote.py`, not a second model) → fixed HTML template opened on the retina |
...
| [docs/lid-awake.md](docs/lid-awake.md) | 😴 Claude insomnia — the `off` / `interactive only` / `background too` submenu, ex-🔋 Claude prevents sleep (`LidAwake` + `LidAwakePolicy` + `ClaudeActivity` + `InternetWatch`): holds the Mac awake on battery with the lid shut **only while a Claude session is working** (a live `caffeinate` with a `claude` parent — or, for a session driven from the phone, which never spawns one, a transcript written in the last five minutes) **and has internet** (5 min without `api.anthropic.com` = every session is parked, let it sleep), via the kernel `SleepDisabled` flag (**why `caffeinate` cannot do this**), the narrow sudoers rule it needs, the 10 s lub-dub from the 💓 SFX that doubles as the proof (system output parked at 100% and unmuted while it runs, both put back before the Mac is let go to sleep), the 🫀 flatline that announces the release, the 🚪 sleep chime that makes silence mean only "something is wrong" (`SleepChime`, on `willSleepNotification`), soft release vs. the hard 20% floor |
| [docs/claude-remote-control.md](docs/claude-remote-control.md) | 🛰️ **Claude RC in background** (`ClaudeRemoteControl`): keeps `claude remote-control` — the server the phone opens new sessions against — alive in the detached `claude-rc` tmux session, rechecked every 60 s and once at launch; the checkbox under 👩🏻‍💻 Extra is the off switch the `ro.victorrentea.claude-rc` LaunchAgent never had (that agent is booted out and `launchctl disable`d, 22 Sep 2026, so two watchdogs cannot race) |
| [docs/home-awake.md](docs/home-awake.md) | 🏠 Home Wi-Fi keeps the screen on (`HomeAwake` + `HomeAwakePolicy` + `DisplaySleepAssertion`): while associated with a configured home SSID the display never idles, so the screen saver → lock path never starts (⌃⌘Q and the lid still lock); CoreWLAN `.ssidDidChange` + a 60 s backstop poll, the SSID in `UserDefaults` not in a branch, why not `caffeinate` and why not 🔋's kernel flag |
...
| [docs/mouse-reconnect.md](docs/mouse-reconnect.md) | 🖱️ **Reconnect Mouse** menu row (`MouseReconnect`) — **manual only, never automatic** (a mouse that reconnects on sight is wrong when it was just put on another computer); **CoreBluetooth**, not `IOBluetoothDevice.openConnection` (that call hangs forever for a BLE-only peripheral — measured, not theorised); scan by name + strongest RSSI rather than an address allow-list, because this mouse's address drifts between pairings |
...
| [docs/displays-projector.md](docs/displays-projector.md) | 🖥️ auto display arrangement (`DisplayArrangementManager`, `KnownDisplays`), 🔴 presentation detection + aggressive 😶😶😶 silent-transcription warning, 🔊 Zoom share-picker prep (`ZoomSharePrep`), ▶️ Zoom join-preview auto-press (`ZoomJoinAutoStart`), 🔍 **which magnifier style the remote can see** (`ZoomLensMode` + `ZoomLensWatch`: full-screen zoom is applied at scanout and never reaches a screen share, the PiP lens does — plus the three dead ends, measured) |
...
| [docs/memory-pressure.md](docs/memory-pressure.md) | 🟥 red plate under the menu bar icon when paging costs CPU (`MemoryPressurePolicy` + `MemoryPressureMonitor`): why the **compressor**, not `vm.swapusage` and not swapins — the 2026-09-17 thrash measurement that settled it; hysteresis + 3-sample sustain; why the warning stays in the menu bar and off the projected retina |
