...
- **📸 Group Photo (break-triggered)** — instead of a fixed clock time, the Group Photo reminder now fires at the **start of a qualifying break**. When a break starts (`MenuBarManager.onBreak` → `AppDelegate`), the pure, unit-tested `GroupPhotoBreakPolicy.shouldPrompt(breakMinutes:at:)` decides: a **lunch** break (≥ 60 min, any time of day) **or** an **afternoon** break (≥ 10 min starting at **13:00** local or later) qualifies; morning coffee breaks are ignored. The prompt only fires when the **training-assistant daemon is connected** (`daemonConnected` — the local WS server has ≥1 client, tracked in `onClientCountChanged`), i.e. there's an audience to photograph. The prompt is **two channels at once** (`promptGroupPhoto`): (1) the primary **on-screen overlay** `GroupPhotoOverlay` — a centered black card with yellow border (title `📸 Group Photo`, subtitle `Let's make some memories? :D`) drawn on the **projected/retina screen**, fades in, holds ~12 s, fades out, click-to-dismiss, latest-wins. Being the app's own always-on-top panel it shows **even while PowerPoint is presenting fullscreen / mirroring to a projector** — which is exactly when macOS suppresses normal notification banners silently into Notification Center. (2) a **persistent native notification** fallback (`interruptionLevel = .timeSensitive`, never auto-removed, id `group-photo:YYYY-MM-DD-HHmm` so each break records its own) — this is the channel macOS may suppress into Notification Center. **Note:** the "Allow Time Sensitive Notifications" toggle never appears for this app (it's locally-signed with no `com.apple.developer.usernotifications.time-sensitive` entitlement, so `.timeSensitive` is downgraded), which is *why* the overlay is the primary channel. Test hook: `GET /test/group-photo` shows the overlay + posts the notification immediately, bypassing the break + connection gates.
...
- `GET /test/group-photo` — show the 📸 Group Photo overlay + post the native notification now, bypassing the break-length + daemon-connected gates (targets the retina/projected screen by design)
