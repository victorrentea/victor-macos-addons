...
| [docs/hotkeys-launchers.md](docs/hotkeys-launchers.md) | 🤖 terminals ⌘⌃C/Q/T + quarter placement + tiling, 📝 ⌘⌃N notes doc, 📕⌘⌃K / 📧⌘⌃G / 📅⌘⌃L openers, ✉️ ⌘⌃M TO DO mail (`GmailCompose`), 📋 ⌘⌃Z/E/R snippet paste, 🎧 ⌘⌃F focus playlist (`FocusPlaylist`), 🎯 ⌘⌃D Walkie Talkie bind, ⌘⌃⌥D dark mode, Wheel×2 given up to Walkie Talkie, 🔎 Cmd+scroll terminal font zoom (`TerminalZoomSizeLock`), 🔄 mouse-wheel scroll reversal (`ScrollReversal`, replaces Scroll Reverser), ⬇️ back thumb button → Return (`BackButtonEnter`, replaces LinearMouse) |
