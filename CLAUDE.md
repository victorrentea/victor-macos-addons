...
| [docs/mouse-reconnect.md](docs/mouse-reconnect.md) | 🖱️ reconnects Victor's own Bluetooth mouse when paired-but-not-connected while nearby (`MouseAutoReconnect`): **CoreBluetooth**, not `IOBluetoothDevice.openConnection` (that call hangs forever for a BLE-only peripheral — measured, not theorised), the classic-address allow-list used only to bootstrap a `CBPeripheral.identifier` once (six identically-named "Logi M650 L" pairings on this Mac, only one his) |
